...
Shell::Shell()
    : transform(glm::mat4(1.f)), animatedTransform(glm::mat4(1.f)), junctureAnimatedTransform(glm::mat4(1.f)) {}
...
void Shell::addCylinder(const std::vector<std::vector<glm::vec4>>& cylinder, glm::vec4 color)
{
    ...
    for (int i = 0; i < cylinder.size() - 1; ++i)
    {
        for (int j = 0; j < cylinder[0].size() - 1; ++j)
        {
            ...
            points.append(MPoint(cylinder[i][j].x, cylinder[i][j].y, cylinder[i][j].z, cylinder[i][j].w));
            points.append(MPoint(cylinder[i][j + 1].x, cylinder[i][j + 1].y, cylinder[i][j + 1].z, cylinder[i][j + 1].w));
            points.append(MPoint(cylinder[i + 1][j + 1].x, cylinder[i + 1][j + 1].y, cylinder[i + 1][j + 1].z, cylinder[i + 1][j + 1].w));
            points.append(MPoint(cylinder[i + 1][j].x, cylinder[i + 1][j].y, cylinder[i + 1][j].z, cylinder[i + 1][j].w));
        ...
        points.append(MPoint(cylinder[i][cylinder[0].size() - 1].x,
            cylinder[i][cylinder[0].size() - 1].y,
            cylinder[i][cylinder[0].size() - 1].z,
            cylinder[i][cylinder[0].size() - 1].w));

        points.append(MPoint(cylinder[i][0].x, cylinder[i][0].y, cylinder[i][0].z, cylinder[i][0].w));

        points.append(MPoint(cylinder[i + 1][0].x, cylinder[i + 1][0].y, cylinder[i + 1][0].z, cylinder[i + 1][0].w));
        points.append(MPoint(cylinder[i + 1][cylinder[0].size() - 1].x,
            cylinder[i + 1][cylinder[0].size() - 1].y,
            cylinder[i + 1][cylinder[0].size() - 1].z,
            cylinder[i + 1][cylinder[0].size() - 1].w));
        vb.insert(vb.end(), vv.begin(), vv.end());
    }
}

int Curve::numImpulses = 10;
Curve::Curve(Cube* parentCube, Cube* childCube) : points(nullptr),
hasAssigned(false), hasTelescope(false), pSegments(nullptr),
extensionState(EXTENDED), extensionExtent(1.f),
parentCube(parentCube), childCube(childCube), endRadius(0.8f), computed(false), processed(false)
{
    for (int i = 0; i < numImpulses; ++i)
        shells.push_back(make_unique<Shell>());
}

Curve::Curve() : parentCube(nullptr), childCube(nullptr), points(nullptr),
hasAssigned(false), hasTelescope(false), pSegments(nullptr),
extensionState(EXTENDED), extensionExtent(1.f), endRadius(0.8f), computed(false), processed(false)
{
    for (int i = 0; i < numImpulses; ++i)
        shells.push_back(make_unique<Shell>());
}
